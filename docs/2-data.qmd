...
To download, go to Information Disclosure → Data Request → Transport Analysis Data. In the selection form, pick Network → Public transit under Step 1, Nationwide under Step 2, and the distribution year under Step 3; the public-transit network is what KTDB distributes as GTFS. The site is Korean only, so use a browser translator to match these menu names and complete the request form.
:::
