...
GTFS uses [interconnected text files](https://gtfs.org/getting-started/features/base/) linked by IDs. `stops.txt` lists station locations, and `routes.txt` defines transit lines. The remaining files describe individual journeys (`trips.txt`), arrival and departure times (`stop_times.txt`), and service days (`calendar.txt`).
...
To download, go to Information Disclosure → Data Request → Transport Analysis Data. In the selection form, pick Network → Public transit under Step 1 and Nationwide under Step 2. Select the distribution year under Step 3. KTDB distributes this public-transit network as GTFS. The site is Korean only, so use a browser translator to match these menu names and complete the request form.
:::
...
Routes were filtered, so dependent tables need updating too. Start with `semi_join()` to keep only trips on the remaining routes. Then retain stop times for those trips and stops that still appear. This cascade is the reverse of spatial clipping, which starts from stops and flows upward.
...
Clipping GTFS requires more than filtering `stops.txt` by coordinates. The tables are linked by IDs, so filtering must cascade. Start with stops inside your bounding box (administrative boundary plus buffer). Then keep the stop times that reference those stops, the trips that appear in those stop times, and the routes that contain those trips.
...
South Korea's administrative hierarchy runs from province (sido) to municipality (sigungu, ~250) to neighborhood (eupmyeondong, ~3,500). For finer resolution, the statistics agency provides enumeration districts (jipgyegu, ~100,000): not administrative units but statistical areas designed for census aggregation.
