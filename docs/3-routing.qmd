...
The pilot inputs are derived from licensed sources: KTDB GTFS, SGIS boundaries, and geocoded bank branches. We therefore do not redistribute them. `NOTICE.md` lists each source and explains how to obtain it. Every output shown in this chapter was produced from this real dataset.

::: {.callout-tip}
## Reproduce this yourself

The repository ships a license-clean synthetic fixture in [`samples/tutorial-synthetic/`](https://github.com/urbanjuhyeon/tracking-access-change/tree/main/samples/tutorial-synthetic). It contains a fictional 7 × 7 street grid with a four-stop bus route, three origins, and three destinations. Generate it with `python samples/tutorial-synthetic/generate_fixture.py`. Then run `run_example.R` to exercise the same `build_network()` → `travel_time_matrix()` path on open data. To follow this chapter with real inputs, prepare your own study area as described in @sec-data.
:::
...
Car, transit, and walk results match within 1--2 minutes. Two settings account for much of the bicycle difference (15.2 vs 6 min). r5r uses 12 km/h, while commercial apps typically use 15--18 km/h. Our LTS=2 restriction also routes cyclists onto safer but longer paths, whereas commercial apps typically optimize for speed on any road.
