...
Scaling extends routing from a single district to **national coverage**. A country-wide network exceeds typical RAM: millions of stops and road segments cannot be processed at once. The solution is to partition national data into district-level packages, route each independently, then merge results: the **split-apply-combine** pattern.
...
r5r loads the entire network into **Java heap memory**. A national network, with millions of transit stops and road segments, exceeds typical RAM limits. The solution: partition national data into smaller units that each fit in memory.
...
The partitioning code in @sec-partitioning produces district packages. The rest of this chapter walks through the workflow on our pre-built sample: 2023 network data for five districts representing diverse urban contexts, from dense metropolitan centers to rural counties. Like the national inputs, the sample embeds licensed data (KTDB GTFS, SGIS boundaries, geocoded bank branches), so we do not redistribute it; every output below is shown as illustration from this real dataset. For a runnable, license-clean starting point, generate the synthetic fixture in [`samples/tutorial-synthetic/`](https://github.com/urbanjuhyeon/tracking-access-change/tree/main/samples/tutorial-synthetic), then apply the code in this section to district packages you prepare for your own study area.
...
1  1 BNK경남은행         거창 127.9065 35.68861
2  2 BNK경남은행         구영 129.2424 35.57147
3  3 BNK경남은행 굴화금융센터 129.2649 35.55462
4  4 BNK경남은행         남목 129.4238 35.53925
5  5 BNK경남은행       남진주 128.0912 35.17945
```
...
**Transit** routing depends on schedules: a trip at 8:00 AM may take 25 minutes, but the same trip at 8:05 could take 40 minutes if you just missed a bus. r5r's `time_window` parameter addresses this by simulating multiple departures and returning the median travel time.
...
For the full national analysis (252 districts), parallel processing dramatically reduces runtime. The `{future}` and `{furrr}` packages distribute work across multiple CPU cores:
...
Now we count destinations reachable within each time threshold. Transit has three rows per origin-destination pair (one per departure slot), so we count per slot first and then average the slots; the other modes have a single (missing) `departure_time` value and pass through unchanged:
...
Accessibility results can change depending on which spatial units are used for analysis, a well-known issue called the Modifiable Areal Unit Problem (MAUP). Our approach mitigates this in two ways:
...
2. **Population-weighted aggregation**: When summarizing to districts, we weight by population rather than averaging across tracts. This ensures results reflect where people actually live, not arbitrary administrative boundaries.
...
- **Time sensitivity**: Extending from 15 to 30 minutes sharply increases transit accessibility in urban areas (Ulsan Jung-gu: 4 → 54), but has minimal effect in rural areas (Hapcheon: 0.5 → 0.7)
